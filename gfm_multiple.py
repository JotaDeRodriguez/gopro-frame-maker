...
class TrekviewCommand():
    ...
    def __subprocess(self, command, sh=0):
        ...
        try:
            ...
            output = subprocess.run(cmd, capture_output=True, shell=bool(sh))
...
class TrekViewGoProMp4(TrekviewPreProcess, TrekviewProcessMp4):
    def __init__(self, args):
        ...
        __configData = {
            ...
            "debug": args.debug,
            "quality": 1,  # Set a default quality
    ...
    def process_directory(self):
        videos = self.find_360_videos()
        for video in videos:
            self.process_single_video(video)

    def find_360_videos(self):
        return glob.glob(os.path.join(self.getConfig()["input_directory"], '*.360'))

    def process_single_video(self, video_path):
        preProcessDataJSON = self._preProcessExifToolExecute(video_path)
        if preProcessDataJSON is None:
            logging.critical(f"Unable to get metadata from video: {video_path}")
            return

        # Add the preProcessDataJSON to the config
        config = self.getConfig()
        config["jsonData"] = preProcessDataJSON
        self.setConfig(config)

        preProcessDataXMLGPS = self._processXMLGPS(video_path)
        if len(preProcessDataXMLGPS) <= 0:
            logging.critical(f"Unable to get GPS metadata from video: {video_path}")
            return

        gps_data = self.extractXMLDataNormal(preProcessDataXMLGPS, [], self.getConfig()["frameRate"], self.getConfig())

        gpx_filename = os.path.splitext(video_path)[0] + '.gpx'
        self.create_gpx_file(gps_data, gpx_filename)

    def create_gpx_file(self, gps_data, gpx_filename):
        gpx = gpxpy.gpx.GPX()
        ...
        for timestamp, point in gps_data.items():
            lat = self.latLngToDecimal(point['GPSLatitude'])
            lon = self.latLngToDecimal(point['GPSLongitude'])
            alt = float(point['GPSAltitude'].split()[0])
            gpx_segment.points.append(
                gpxpy.gpx.GPXTrackPoint(latitude=lat, longitude=lon, elevation=alt, time=timestamp))

        with open(gpx_filename, 'w') as f:
            f.write(gpx.to_xml())
        logging.info(f"Created GPX file: {gpx_filename}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("input_directory", type=str, help="Input directory containing .360 video files.")
    ...
    goProMp4.process_directory()
    print("Processing complete. GPX files have been created for each .360 video.")